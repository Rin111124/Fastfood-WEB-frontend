...
const CustomerDashboard = () => {
  ...
  const handleProfileSubmit = async (event) => {
    ...
    try {
      ...
      setStatusMessage("Đã cập nhật thông tin cá nhân")
  ...
  const handleOrderSubmit = async (event) => {
    ...
    try {
      ...
      setStatusMessage("Đã tạo đơn hàng mới!")
      setStatusType("success")
  ...
  const handleCancelOrder = async (orderId) => {
    try {
      ...
      setStatusMessage(`Đã hủy đơn hàng #${orderId}`)
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-4 mb-4">
        <div>
          <p className="text-muted mb-1 text-uppercase small fw-semibold">Tổng quan khách hàng</p>
          <h1 className="display-6 fw-bold mb-2">Xin chào, {customerName}!</h1>
          <p className="text-secondary mb-0">
            Theo dõi đơn hàng, ưu đãi và gợi ý món ăn được cá nhân hóa cho bạn.
          </p>
        </div>
        {favoriteCategory && (
          <div className="badge bg-warning-subtle text-warning px-3 py-2 rounded-pill text-uppercase">
            Yêu thích: {favoriteCategory}
          </div>
        )}
      </div>
            ...
            <div className="col-lg-5">
              <div className="card shadow-sm border-0 h-100">
                <div className="card-header bg-white border-0 pb-0">
                  <p className="text-muted text-uppercase small fw-semibold mb-1">Thông tin cá nhân</p>
                  <h5 className="mb-0">Cập nhật hồ sơ</h5>
                </div>
                <div className="card-body">
                  <form className="row g-3" onSubmit={handleProfileSubmit}>
                    <div className="col-md-6">
                      <label className="form-label text-muted small text-uppercase">Họ tên</label>
                      <input
                        type="text"
                    ...
                    <div className="col-md-6">
                      <label className="form-label text-muted small text-uppercase">Số điện thoại</label>
                      <input
                        type="tel"
                        className="form-control"
                          ...
                          {recentOrders.map((order) => (
                            <tr key={order.order_id}>
                              ...
                              <td className="text-muted small">
                                {order.items?.length
                                  ? order.items
                                      .map((item) => `${item.quantity}x ${item.product?.name || "Sản phẩm"}`)
                                      .join(", ")
                                  : "Chưa có món ăn"}
                              </td>
                              <td className="text-muted small">
